...
public class TabuleiroTests
{
    ...
    /// </summary>
    [Fact(DisplayName = "Deve retornar corretamente as pontas extremas após múltiplas peças coladas (Critério: ponta esquerda = ValorA da primeira peça, ponta direita = ValorB da última peça). O aluno deve garantir a ordem e o cálculo das pontas.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve retornar a soma correta das pontas externas com uma peça (Critério: soma = ValorA + ValorB). O aluno deve garantir o funcionamento para o caso mais simples.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve retornar a soma apenas das pontas extremas com múltiplas peças (Critério: soma = ValorA da primeira + ValorB da última). O aluno deve garantir que valores intermediários não sejam somados.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve zerar a soma das pontas após limpar o tabuleiro (Critério: soma antes > 0, depois = 0). O aluno deve garantir que Limpar remova todas as peças e resete o estado.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Simula sequência de jogadas e verifica soma das pontas após cada jogada (Critério: soma correta a cada jogada). O aluno deve garantir atualização dinâmica da soma.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Verifica se as pontas mudam corretamente após cada nova peça (Critério: pontas atualizadas após cada jogada). O aluno deve garantir cálculo dinâmico das pontas.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve permitir colar a primeira peça em qualquer lado (Critério: PodeColar true para ambos os lados se vazio). O aluno deve garantir validação correta do estado inicial.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve permitir colar apenas peças compatíveis com a ponta do lado escolhido (Critério: PodeColar true só se valor bate com a ponta). O aluno deve garantir validação para ambos os lados.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve adicionar peças no lado correto, respeitando as regras do dominó (Critério: pontas refletem valores esperados após colagens). O aluno deve garantir ordem e inversão corretas.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Deve retornar true para travamento quando nenhuma mão pode jogar (Critério: EstaTravado true se nenhuma mão tem peça compatível). O aluno deve garantir análise de todas as mãos e pontas.")]
    ...
    /// <br/><b>O que o aluno deve garantir:</b> A lógica deve considerar todas as mãos e pontas do tabuleiro.
    /// </summary>
    [Fact(DisplayName = "Deve retornar false para travamento se ao menos uma mão pode jogar (Critério: EstaTravado false se alguma mão tem peça compatível). O aluno deve garantir análise de todas as mãos e pontas.")]
    ...
    /// <br/><b>O que o aluno deve garantir:</b> O método Colar deve manipular corretamente a ordem e inversão das peças.
    /// </summary>
    [Fact(DisplayName = "Simula sequência de colagens e verifica se as pontas mudam corretamente após cada jogada (Critério: mudança de estado correta após cada jogada). O aluno deve garantir ordem e inversão corretas.")]
    ...
    /// </summary>
    [Fact(DisplayName = "Simula jogadas até travar o tabuleiro e depois destrava ao adicionar uma mão compatível (Critério: EstaTravado muda de true para false conforme as mãos). O aluno deve garantir lógica dinâmica.")]
